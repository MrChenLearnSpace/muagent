...
交互终端支持与 Codex/Claude Code 类似的操作：用户输入使用亮青色粗体 `YOU ›`，Agent 输出使用亮绿色 `AGENT ›`；`Enter` 在编辑区插入换行，`Shift+Enter` 才提交整段内容；上下方向键可浏览本次运行已经提交的输入历史。输入 `/` 后按 `Tab` 显示所有候选；输入命令前缀后按 `Tab`，唯一候选会自动补齐，多个候选会先扩展共同前缀、再次按 `Tab` 显示候选清单。输入被管道重定向时无法识别组合键，因此自动退回普通逐行读取。

Agent 的 Markdown 不再原样显示标记：标题、粗体、行内代码、链接、列表、引用、分隔线和围栏代码块会转换为 ANSI 颜色及终端符号；设置标准 `NO_COLOR` 环境变量或把输出重定向到文件时自动使用无颜色纯文本。模型输出中的其他控制字符会被过滤，避免意外改变终端状态。
...
模型生成较大的单文件时，工具参数可能在 `MaxOutputTokens` 处被截断。协议适配器会在落库前验证每个工具参数必须是完整 JSON 对象；残缺参数会被替换为可安全回传的错误结果，模型随后应把页面拆为较小的 HTML、CSS、JavaScript 文件重试。旧会话中已保存的残缺 JSON 也会在下一次对话时自动修复，因此不会再把非法历史发送给 Responses 服务并引发 HTTP 500。
