...
交互终端支持与 Codex/Claude Code 类似的操作：用户输入使用亮青色粗体 `YOU ›`，Agent 输出使用亮绿色 `AGENT ›`；`Enter` 提交整段内容，`Shift+Enter` 在编辑区插入换行；上下方向键可浏览本次运行已经提交的输入历史。输入 `/` 后按 `Tab` 显示所有候选；输入命令前缀后按 `Tab`，唯一候选会自动补齐，多个候选会先扩展共同前缀、再次按 `Tab` 显示候选清单。输入被管道重定向时无法识别组合键，因此自动退回普通逐行读取。
